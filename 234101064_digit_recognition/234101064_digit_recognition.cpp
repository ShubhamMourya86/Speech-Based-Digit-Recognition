// 234101064_digit_recognition.cpp : Defines the entry point for the console application.
...
void training(){
	...
	for(int d = 0; d<=9; d++){
		...
		for(int u = 11; u <= 10 + total_files_trained; u++){
			...
			sprintf(filename, "input/recordings/Digit %d/234101064_E_%d_%d.txt", d,d, u);
...
void validation(){
	...
	sprintf(filename, "input/recordings/Digit 0/234101064_E_1_1.txt");
...
void testing(){
	...
	for(int d = 0; d<=9; d++){
		...
		for(int j = 1; j<=10; j++){
			sprintf(filename, "input/recordings/Digit %d/234101064_E_%d_%d.txt", d,d, j);
